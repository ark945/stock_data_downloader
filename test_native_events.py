...
save_dir = os.path.join(os.path.dirname(__file__), "downloads_tpex_local", "test_native_events")
...
for code in test_codes:
    ...
    # 2. 用真實原生鍵盤輸入並按 Enter 鍵查詢 (100% 觸發 jQuery AJAX 且不跳頁)
    stk_input = page.ele("css:input.code")
    stk_input.input(f"{code}\n", clear=True)
    time.sleep(1.5)

    # 3. 原生真實滑鼠點擊下載按鈕
    d_btn = page.ele('css:button[data-format="utf-8"]')
    if d_btn:
        d_btn.click(by_js=False)
        found = False
        for _ in range(15):
            time.sleep(0.4)
            ...
            if csvs:
                ...
                print(f"[✓ 原生事件成功] {code} 下載完成！檔案: {os.path.basename(csvs[0])} 大小: {fsize} bytes")
                sys.stdout.flush()
                results[code] = fsize
                found = True
                break
