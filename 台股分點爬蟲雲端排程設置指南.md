      ...
      - name: 執行全市場分點爬蟲 (5 輪安全補抓 + Email 短缺日報)
        ...
        run: |
          ...
          if [ -n "$TARGET_DATE" ]; then
            python "$TARGET_SCRIPT" --market "$MARKET" --date "$TARGET_DATE" --workers 2 --max-rounds 5 --no-excel
          else
            python "$TARGET_SCRIPT" --market "$MARKET" --workers 2 --max-rounds 5 --no-excel
          fi
...
### 1.2 設定 Email 報表通知 (GitHub Secrets)

爬蟲結束後若欲自動收到 **HTML 視覺化執行成果與短缺股票明細日報**，請在 GitHub 倉庫設定 Secrets：

1. 前往 GitHub 專案頁面 ➡️ **Settings** ➡️ **Secrets and variables** ➡️ **Actions**。
2. 點擊 **New repository secret**，新增以下 3 個變數：

| Secret 名稱 | 內容說明 | 範例 |
| :--- | :--- | :--- |
| **`SMTP_USER`** | 您的寄件 Gmail / 企業信箱 | `yourname@gmail.com` |
| **`SMTP_PASSWORD`** | Google 產生的 16 位應用程式密碼 | `abcd efgh ijkl mnop` |
| **`RECEIVER_EMAIL`** | 接收短缺日報的信箱 | `ark@tdcc.com.tw` |

> [!TIP]
> **如何取得 Google 應用程式密碼 (App Password)？**
> 1. 前往 [Google 應用程式密碼產生頁面](https://myaccount.google.com/apppasswords)（需先啟用兩步驟驗證）。
> 2. 應用程式名稱輸入「台股爬蟲通知」，點擊「建立」。
> 3. 複製彈出的 16 位英文字母密碼，填入 `SMTP_PASSWORD`。
