...
class TPEXBrokerCrawler:
    ...
    def crawl_all_stocks_session(
        ...
        """
        使用單一持久化 Chromium 瀏覽器實例連續批次抓取全市場上櫃股票；以單檔下載檔案管理、DOM 重用與分段 profiling 降低固定成本
        """
        try:
        ...
        try:
            ...
            for idx, sym in enumerate(stock_codes, 1):
                try:
                    ...
                    # 1. 只清除「本檔股票」的舊 CSV，不掃描/刪除整個下載目錄。
                    #    這可避免每檔 glob + delete 的固定成本，也為未來多 Worker 做準備。
                    ...
                    # 2. 尋找輸入框並填入代號
                    #    正常情況下 DOM 不會重建，因此先重用已找到的 element；
                    #    若頁面失效，再重新定位。
                    if 'stk_input' not in locals() or not stk_input:
                        stk_input = page.ele("css:input.code", timeout=2) or page.ele("@name=code", timeout=2)
                    if not stk_input:
                    ...
                    input_elapsed = time.perf_counter() - t_input

                    # 3. 點擊查詢按鈕
                    if 'q_btn' not in locals() or not q_btn:
                    ...
                    if q_btn:
                        ...
                        print(f"[!] {sym} 查無查詢按鈕")
                        continue
                    query_click_elapsed = time.perf_counter() - t_query

                    # 4. 點擊下載按鈕並使用官方原生 to_download 等待機制。
                    #    不再額外固定 sleep(0.2)，直接等待下載按鈕出現。
                    t_download = time.perf_counter()
                    d_btn = page.ele("text:下載 CSV (UTF-8)", timeout=1) or page.ele("text:下載 CSV", timeout=1)
                    find_download_elapsed = time.perf_counter() - t_download

                    if d_btn:
                        t_wait = time.perf_counter()
                        mission = d_btn.click.to_download(save_path=save_dir, rename=f"{sym}.csv")
                        mission.wait(show=False, timeout=2.5)
                        download_wait_elapsed = time.perf_counter() - t_wait
                        ...
                        if os.path.exists(target_csv_file):
                            ...
                            if df is not None and not df.empty:
                                ...
                                print(
                                    f"[{ts_res}]   [上櫃 {idx}/{total}] [OK] {sym} ({len(df)} 筆) "
                                    f"| 本檔 {stock_elapsed:.2f}s "
                                    f"| input {input_elapsed:.2f}s "
                                    f"| query {query_click_elapsed:.2f}s "
                                    f"| findDL {find_download_elapsed:.2f}s "
                                    f"| download {download_wait_elapsed:.2f}s "
                                    f"| parse {parse_elapsed:.2f}s "
                                    f"| 平均 {1/speed:.2f}s/檔 "
                                    f"| 剩餘約: {remain/60:.1f} 分鐘"
                                )
                            else:
                                failed_symbols.append(sym)
