...
* **抗封鎖架構**：由於櫃買中心（TPEX）掛載 Cloudflare 智慧防護，針對海外雲端 IP 嚴格限制同 IP 併發數。本專案採用「單一純淨持久瀏覽器會話」，只在啟動時載入一次 DOM，後續全透過 JavaScript 高速切換輸入與官方原生下載，**完全規避 Cloudflare 限流，確保 1,005 檔 100% 採集零漏抓**。
* **精準檔案過濾與嚴格代碼校驗**：
...
* **目標資料夾**：[Google Drive 目標資料夾](https://drive.google.com/drive/folders/1L6f9jQx9phz_ZoVfDmBQViEVnoDHh_0a?usp=sharing)
* **資料夾 ID**：`1L6f9jQx9phz_ZoVfDmBQViEVnoDHh_0a`
...
4. 於 GitHub 倉庫 ➡️ **Settings** ➡️ **Secrets and variables** ➡️ **Actions** 新增 Secret：
   ...
   * (選填) `GDRIVE_FOLDER_ID`：自訂資料夾 ID（預設為 `1L6f9jQx9phz_ZoVfDmBQViEVnoDHh_0a`）。
