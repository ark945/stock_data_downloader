...
### 7.1 持久化瀏覽器會話 (Single-Tab Persistent Session)
* **架構創新**：相較於傳統每檔股票重啟 Chrome 需耗費 13 秒/檔（抓全市場需 3.6 小時），本專案採用「單一持久化瀏覽器會話」，只在啟動時載入一次 DOM，後續全透過 JavaScript 高速切換輸入與下載，**每檔壓縮至 1.5 ~ 2.0 秒（提速 6~8 倍）**。
* **精準檔案過濾與嚴格代碼校驗**：
  1. **零舊檔殘留**：每檔下載前強制清空暫存目錄。
