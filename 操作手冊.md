...
## 9. Google Drive 雲端硬碟自動同步整合 (Google Drive Sync)

本系統支援在全市場 Parquet 成果產出後，**全自動無人值守上傳至指定 Google Drive 資料夾**，並於 Telegram / Email 推播中附上即時檢視與下載連結。

* **目標資料夾**：由您自行在 Google Drive 建立並填入 GitHub Secret `GDRIVE_FOLDER_ID`
* **資料夾 ID**：`<YOUR_GDRIVE_FOLDER_ID>`（於資料夾網址列 `folders/` 後方那串英數字）

### 9.1 三分鐘啟用 Google Cloud 服務帳戶 (Service Account)：
1. 前往 [Google Cloud Console](https://console.cloud.google.com/) 啟用 **Google Drive API**。
2. 於「憑證 (Credentials)」建立一個**服務帳戶 (Service Account)**，並建立一組 **JSON 格式金鑰**（下載該 JSON 檔案）。
3. 將該服務帳戶之 Email（例如 `crawler-sa@project.iam.gserviceaccount.com`）透過 Google Drive 右上角「共用」加入為目標資料夾的 **「編輯者 (Editor)」**。
4. 於 GitHub 倉庫 ➡️ **Settings** ➡️ **Secrets and variables** ➡️ **Actions** 新增 Secret：
   * `GDRIVE_SERVICE_ACCOUNT_KEY`：貼上該 JSON 金鑰之完整字串內容。
   * `GDRIVE_FOLDER_ID`：您的目標資料夾 ID（必填，於 Google Drive 資料夾網址列 `folders/` 後方那串英數字）。
