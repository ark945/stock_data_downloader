...
def get_latest_trading_date() -> str:
    """
    精確計算台股最新交易日 (官方於每日 16:30 確定提供最新券商日報表)：
    - 週六 (5)：退回週五 (-1 天)
    - 週日 (6)：退回週五 (-2 天)
    - 週一 (0) 且未過 16:30 (盤前/盤中)：退回上週五 (-3 天)
    - 週二至週五 (1~4) 且未過 16:30：退回前一天 (-1 天)
    - 週一至週五 且已過 16:30 (盤後已就緒)：當天 (-0 天)
    """
    now = get_taipei_now()
    w = now.weekday()
    is_ready = (now.hour, now.minute) >= (16, 30)
    if w == 5:
        delta = 1
    elif w == 6:
        delta = 2
...
def run_full_market_crawler(
    ...
    if markets in ["all", "twse"]:
        ...
        log_msg(f">>> [階段 1/2] 啟動 TWSE 上市股票分點抓取 ({actual_twse_w} Workers, 最多 {max_rounds} 輪安全補抓)...")
...
def main():
    parser = argparse.ArgumentParser(description="全市場台股分點買賣日報表爬蟲協調控制器 (含多輪補抓與 Email 短缺通知)")
    ...
    parser.add_argument("--max-rounds", type=int, default=10, help="上市最大安全補抓輪數 (預設 10 輪，含最終深層收斂)")
